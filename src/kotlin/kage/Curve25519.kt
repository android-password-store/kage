...
class Curve25519 {
    ...
    // Allocate memory for all the temporary variables we will need.
    private val A: IntArray = IntArray(NUM_LIMBS_255BIT)
    private val AA: IntArray = IntArray(NUM_LIMBS_255BIT)
    private val B: IntArray = IntArray(NUM_LIMBS_255BIT)
    private val BB: IntArray = IntArray(NUM_LIMBS_255BIT)
    private val C: IntArray = IntArray(NUM_LIMBS_255BIT)
    private val CB: IntArray = IntArray(NUM_LIMBS_255BIT)
    private val D: IntArray = IntArray(NUM_LIMBS_255BIT)
    private val DA: IntArray = IntArray(NUM_LIMBS_255BIT)
    private val E: IntArray = IntArray(NUM_LIMBS_255BIT)
    private val t1: LongArray = LongArray(NUM_LIMBS_510BIT)
    private val t2: IntArray = IntArray(NUM_LIMBS_510BIT)
    private val x_1: IntArray = IntArray(NUM_LIMBS_255BIT)
    private val x_2: IntArray = IntArray(NUM_LIMBS_255BIT)
    private val x_3: IntArray = IntArray(NUM_LIMBS_255BIT)
    private val z_2: IntArray = IntArray(NUM_LIMBS_255BIT)
    private val z_3: IntArray = IntArray(NUM_LIMBS_255BIT)
            ...
            try {
                ...
                if (publicKey != null) {
                    ...
                    for (index in 0 until 32) {
                        val bit = (index * 8) % 26
                        val word = (index * 8) / 26
                        val value: Int = publicKey[index].toInt() and 0xFF
                        if (bit <= (26 - 8)) {
    ...
    private fun evalCurve(s: ByteArray) {
        var sposn = 31
        var sbit = 6
        var svalue: Int = s[sposn].toInt() or 0x40
        var swap = 0
    ...
    private fun reduceQuick(x: IntArray) {
        ...
        for (index in 0 until NUM_LIMBS_255BIT) x[index] = (x[index] and nmask) or (t2[index] and mask)
    }
